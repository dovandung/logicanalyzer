...
namespace LogicAnalyzer.Controls
{
    public partial class SampleViewer : UserControl, ISampleDisplay, IRegionDisplay, IMarkerDisplay
    {
        ...
        public int? UserMarker1 { get; private set; }
        public int? UserMarker2 { get; private set; }
        public int? marker_switch { get; private set; }
        public double sFrequency;
        bool updating = false;
        ...
        public SampleViewer()
        {
            ...
            this.PointerPressed += SampleViewer_MousePress ;
        }
        private void SampleViewer_MousePress(object? sender, PointerEventArgs e)
            {
            int i;
            i = 0;
            
            double sampleWidth = Bounds.Width / (double)VisibleSamples;
            var ovrSample = (int)(e.GetPosition(this).X / sampleWidth) + FirstSample;
            if (marker_switch == 0)
            {
                this.UserMarker1 = ovrSample;
                marker_switch = 1;
            }
            else
            {
                this.UserMarker2 = ovrSample;
                marker_switch = 0;
            }
            if ((this.UserMarker1 != null)&&(this.UserMarker2!=null))
            {
                double tt = (double )Math.Abs((decimal)(UserMarker1 - UserMarker2));
                //Mea1.Text =(tt/sFrequency).ToString();
                Mea1.Text = (tt / sFrequency).ToSmallTime();
            }
            if (!updating)
        ...
        {
            channels = Channels;
            sFrequency = (double)SampleFrequency;
        ...
        {  /*
            if (marker_switch == 0)
            {
                this.UserMarker1 = UserMarker;
                marker_switch = 1;
            }
            else
            {
                this.UserMarker2 = UserMarker;
                marker_switch = 0;
            }
            if (!updating)
                this.InvalidateVisual(); */
            ...
            using (context.PushClip(thisBounds))
            {
                ...
                if (regions.Count > 0)
                {
                    foreach (var region in regions)
                    {
                        ...
                        //context.DrawLine(GraphicObjectsCache.GetPen(AnalyzerColors.UserLineColor, 2, DashStyle.DashDot), new Point(start, 0), new Point(start, thisBounds.Height));
                        //context.DrawLine(GraphicObjectsCache.GetPen(AnalyzerColors.UserLineColor, 2, DashStyle.DashDot), new Point(end, 0), new Point(end, thisBounds.Height));
                    }
                }
                ...
                for (int curSample = FirstSample; curSample < lastSample; curSample++)
                {
                    ...
                    if (UserMarker1 != null && UserMarker1 == curSample)
                    ...
                    if (UserMarker2 != null && UserMarker2 == curSample)
                        context.DrawLine(GraphicObjectsCache.GetPen(AnalyzerColors.UserLineColor, 2, DashStyle.DashDot), new Point(lineX, 0), new Point(lineX, thisBounds.Height));
                ...
                if (UserMarker1 != null && UserMarker1 == lastSample)
                {
                    double lineX = (lastSample - FirstSample) * sampleWidth;
                    context.DrawLine(GraphicObjectsCache.GetPen(AnalyzerColors.UserLineColor, 2, DashStyle.DashDot), new Point(lineX, 0), new Point(lineX, thisBounds.Height));
                }

                if (UserMarker2 != null && UserMarker2 == lastSample)
                {
                    double lineX = (lastSample - FirstSample) * sampleWidth;
                    context.DrawLine(GraphicObjectsCache.GetPen(AnalyzerColors.UserLineColor, 2, DashStyle.DashDot), new Point(lineX, 0), new Point(lineX, thisBounds.Height));
                }
